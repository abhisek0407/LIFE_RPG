...
export default function App() {
  ...
  const [storeItems, setStoreItems] = useState(() => storageService.getStoreItems());
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [userData, questsData, dailiesData, storeData] = await Promise.all([
        ...
        console.warn('Backend unavailable, running on local persistent state', err);
      }
    }
    loadData();
  }, []);
  ...
  const handleCompleteMicrotask = async ({ questId, microtaskId, domain, xp, gold, isQuestFinished }) => {
  ...
  const handleCompleteGroundingTask = async ({ taskTitle, domain, xp, gold }) => {
    const result = await apiService.completeMicrotask({
    ...
    if (result.overallLeveledUp || result.domainLeveledUp) {
      setLevelUpData({
        ...
        newLevel: result.domainLeveledUp ? result.newDomainLevel : result.newOverallLevel,
  ...
  const handleToggleDaily = async (daily) => {
    ...
    setDailies((prev) =>
      prev.map((d) =>
        d.id === daily.id
          ? { ...d, isCompletedToday: true, streakDays: (d.streakDays || 0) + 1 }
          : d
      )
    );
