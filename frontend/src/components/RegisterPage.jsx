...
export default function RegisterPage({ onRegister, onSwitchToLogin }) {
  ...
  // True once the account is actually created on the server. Some users hit
  // a blank screen right after registration because handing the freshly
  // created user object straight into the running app's state can land
  // before every dependent piece of state (quests/dailies/store, still
  // loading) is ready. A plain page reload is what a manual refresh already
  // does to fix it — so once we know the account exists, we do that reload
  // ourselves instead of making the user find the browser's refresh button.
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // The token is already saved to localStorage inside apiService.register
      // at this point. Rather than hand result.user into onRegister() and
      // hope every component the dashboard depends on is happy with a
      // brand-new, still-loading state, reload the page: that re-runs
      // App's restoreSession() from the stored token — the exact path a
      // manual browser refresh takes, which reliably works.
      setSuccess(true);
    ...
    <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-md">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl">
          {success ? (
            // Brief confirmation screen while the automatic reload kicks in.
            // The "Refresh now" button is a manual fallback in case the
            // timer is delayed (e.g. a throttled background tab).
            <div className="text-center py-6">
              <div className="mx-auto w-14 h-14 rounded-2xl bg-emerald-500/15 border border-emerald-500/30 flex items-center justify-center mb-5">
                <Sparkles className="w-7 h-7 text-emerald-400" />
              </div>
              <h2 className="text-lg font-semibold mb-1">Account created!</h2>
              <p className="text-slate-400 text-sm mb-6">
                Loading your hero into LifeRPG...
              </p>
              <button
                type="button"
