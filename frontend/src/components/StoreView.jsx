...
export default function StoreView({ user, storeItems, onBuyItem }) {
  ...
  const handleBuy = (item) => {
    if (userGold < item.costGold) {
      alert(`Not enough Gold! You need ${item.costGold} Gold (you have ${userGold}). Complete more microtasks to earn Gold!`);
      return;
    }
    ...
    <div className="space-y-6 transition-colors duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 sm:p-8 rounded-3xl bg-gradient-to-r from-amber-100 via-yellow-50 to-amber-100 dark:from-amber-950/40 dark:via-rpg-card dark:to-yellow-950/30 border border-amber-200 dark:border-amber-500/30 shadow-sm">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-slate-500 dark:text-slate-400">Virtual Perks, Buffs & Badges</span>
          </div>
          <h2 className="text-2xl sm:text-3xl font-heading font-extrabold text-slate-900 dark:text-white">
            Store & Rewards
          </h2>
          <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 mt-1">
            Exchange your hard-earned gold for real game boosts, cosmetics, and streak insurance.
          </p>
        </div>
        <div className="flex flex-col sm:items-end gap-3">
          <div className="flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-amber-50 dark:bg-amber-500/10 border border-amber-300 dark:border-amber-500/40 shadow-sm dark:shadow-glow-gold">
            ...
            <span className="text-xs font-semibold text-amber-700 dark:text-amber-400/80 uppercase">Gold Available</span>
          </div>
      ...
      {activeSubTab === 'catalog' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {storeItems.map((item) => {
            const canAfford = userGold >= item.costGold;
            const alreadyOwned = inventory.some((inv) => inv.itemId === item.id && item.type !== 'potion');
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded border ${getRarityBadge(item.rarity)}`}>
                      {item.rarity}
                    </span>
                    <div className="flex items-center gap-1 text-amber-700 dark:text-amber-300 font-mono font-bold text-sm">
                ...
                >
                  {alreadyOwned ? (
                    ...
                    <>
                      ...
                      <span>{canAfford ? `Acquire for ${item.costGold}g` : 'Need More Gold'}</span>
                    </>
                  )}
                </button>
              </div>
        ...
        <div className="space-y-4">
          {inventory.length === 0 ? (
            <div className="p-10 rounded-2xl bg-white dark:bg-rpg-card border border-slate-200 dark:border-rpg-border text-center text-slate-500 dark:text-slate-400 shadow-sm">
              ...
              <p className="text-sm font-semibold text-slate-900 dark:text-white">Your inventory is currently empty</p>
              <p className="text-xs mt-1">Conquer quests to earn gold and purchase items from the shop!</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {inventory.map((inv, idx) => (
                <div key={idx} className="p-4 rounded-2xl bg-white dark:bg-rpg-card border border-slate-200 dark:border-rpg-border flex items-center justify-between shadow-sm">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-slate-900 dark:text-white">{inv.name}</h4>
                      <p className="text-xs text-slate-500 dark:text-slate-400">Quantity: {inv.quantity || 1}</p>
                    </div>
                  </div>
