...
export default function TopNavBar({
  ...
  const domainList = [
    { key: 'mental', config: DOMAINS.mental, data: domains?.mental || { level: 1, currentXp: 0, xpToNextLevel: 100 } },
    { key: 'health', config: DOMAINS.health, data: domains?.health || { level: 1, currentXp: 0, xpToNextLevel: 100 } },
    { key: 'skill',  config: DOMAINS.skill,  data: domains?.skill  || { level: 1, currentXp: 0, xpToNextLevel: 100 } },
  ];
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <div className="flex sm:hidden items-center gap-1.5 shrink-0">
          <div className="flex items-center gap-1 px-2 py-1.5 rounded-xl bg-slate-100 dark:bg-rpg-card/80 border border-slate-200 dark:border-rpg-border">
            ...
            <span className="font-mono font-bold text-[11px] text-amber-700 dark:text-amber-300">{character?.gold ?? 0}</span>
          </div>
          <button
            type="button"
          ...
          >
            ...
            <span className="font-mono font-bold text-[11px] text-orange-700 dark:text-orange-300">{streak?.currentStreak ?? 1}</span>
            <span className="text-[9px] font-mono font-bold px-1 py-0.5 rounded bg-orange-200 dark:bg-orange-500/20 text-orange-800 dark:text-orange-200">{streakMultiplier}x</span>
          </button>
        </div>
        ...
        <div className="hidden sm:flex flex-col shrink-0 min-w-[96px] bg-slate-100 dark:bg-rpg-card/80 border border-slate-200 dark:border-rpg-border rounded-2xl px-3.5 py-2.5 gap-2.5">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono font-bold text-xs text-amber-700 dark:text-amber-300 leading-none">{character?.gold ?? 0}</span>
            <span className="text-[10px] uppercase text-amber-600/80 dark:text-amber-400/70 font-semibold leading-none tracking-wide">Gold</span>
          </div>
          ...
          >
            ...
            <span className="font-mono font-bold text-xs text-orange-700 dark:text-orange-300 leading-none">{streak?.currentStreak ?? 1}</span>
            <span className="text-[10px] text-orange-600/80 dark:text-orange-400/70 font-semibold leading-none">Streak</span>
            <span className="text-[9px] font-mono font-bold px-1 py-0.5 rounded bg-orange-200 dark:bg-orange-500/20 text-orange-800 dark:text-orange-200 leading-none ml-auto">{streakMultiplier}x</span>
          </button>
        </div>
        ...
        <div className="hidden sm:grid grid-cols-3 gap-2.5 sm:gap-3 flex-1 max-w-xl mx-auto">
          {domainList.map(({ key, config, data }) => {
            const pct  = Math.min(100, Math.round((data.currentXp / Math.max(1, data.xpToNextLevel)) * 100));
              ...
              >
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-1.5 min-w-0">
                    <div className="p-1 rounded shrink-0" style={{ color: config.color }}>
                  ...
                  <span className="text-[10px] font-bold px-1.5 py-0.5 rounded font-mono shrink-0 ml-1" style={{ backgroundColor: `${config.color}22`, color: config.color }}>
                    Lv.{data.level}
                  </span>
                </div>
                <div className="w-full bg-slate-200 dark:bg-slate-800/80 rounded-full h-2 overflow-hidden">
                  <div className="h-full rounded-full transition-all duration-500 ease-out" style={{ width: `${pct}%`, backgroundColor: config.color, boxShadow: `0 0 6px ${config.color}70` }} />
                </div>
                ...
                <div className="hidden group-hover:block absolute top-full left-0 mt-2 z-40 w-52 p-2.5 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-300 text-xs rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 pointer-events-none">
                  <p className="font-semibold text-slate-900 dark:text-white mb-0.5">{config.subtitle}</p>
        ...
        <div className="relative shrink-0" ref={dropdownRef}>
          ...
          >
            ...
            <div className="relative">
              {avatarUrl ? (
                <img src={avatarUrl} alt={displayName} className="w-8 h-8 sm:w-10 sm:h-10 rounded-full object-cover border-2 border-cyan-400/60 dark:border-cyan-500/50" />
              ) : (
            ...
            <div className="hidden sm:flex flex-col items-start leading-tight">
              <span className="text-xs font-bold text-slate-800 dark:text-slate-100 truncate max-w-[100px]">{displayName}</span>
              <span className="text-[10px] text-slate-400 dark:text-slate-500 font-mono truncate max-w-[120px]">{displayEmail}</span>
            </div>
            ...
            <div className="absolute right-0 top-full mt-2 w-[calc(100vw-24px)] max-w-xs sm:w-64 bg-white dark:bg-rpg-dark rounded-2xl border border-slate-200 dark:border-rpg-border shadow-2xl dark:shadow-black/50 overflow-hidden z-50">
              ...
              <div className="px-4 py-3.5 bg-gradient-to-r from-cyan-50 to-indigo-50 dark:from-cyan-950/40 dark:to-indigo-950/40 border-b border-slate-200 dark:border-rpg-border">
                <div className="flex items-center gap-3">
                  <div className="relative">
                    {avatarUrl ? (
                      <img src={avatarUrl} alt={displayName} className="w-10 h-10 rounded-full object-cover border-2 border-cyan-400/60" />
                    ) : (
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 font-mono truncate">{displayEmail}</p>
                    <div className="flex items-center gap-1.5 mt-1">
                      <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-cyan-500/15 text-cyan-700 dark:text-cyan-300 border border-cyan-300 dark:border-cyan-500/30 font-mono">Lv.{overallLevel}</span>
              ...
              <div className="px-2 py-2 border-b border-slate-100 dark:border-rpg-border">
                ...
                <button
                  type="button"
                  onClick={() => { setDropdownOpen(false); if (onOpenPersonaTab) onOpenPersonaTab(); }}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-slate-800 dark:text-slate-200">Character & Persona</p>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 font-mono truncate">{character?.title || 'Novice Adventurer'}</p>
                  </div>
                </button>
              </div>
              ...
              <div className="px-2 py-2 border-b border-slate-100 dark:border-rpg-border">
                ...
                <div className="flex items-center justify-between px-3 py-2.5 rounded-xl hover:bg-slate-100 dark:hover:bg-rpg-card transition-colors">
                  <div className="flex items-center gap-2.5">
                    {soundEnabled ? <Volume2 className="w-4 h-4 text-slate-500 dark:text-slate-400 shrink-0" /> : <VolumeX className="w-4 h-4 text-slate-400 dark:text-slate-500 shrink-0" />}
                    <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">Sound FX</span>
                  </div>
                  <button
                  ...
                  >
                    <span className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white shadow-sm transition-transform duration-200 ${soundEnabled ? 'translate-x-5' : 'translate-x-0'}`} />
                  </button>
                </div>
                {/* Theme */}
                <div className="flex items-center justify-between px-3 py-2.5 rounded-xl hover:bg-slate-100 dark:hover:bg-rpg-card transition-colors">
                  <div className="flex items-center gap-2.5">
                    {theme === 'dark' ? <Moon className="w-4 h-4 text-indigo-400 shrink-0" /> : <Sun className="w-4 h-4 text-amber-500 shrink-0" />}
                    <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">{theme === 'dark' ? 'Dark Mode' : 'Light Mode'}</span>
                  </div>
                  <button
                    onClick={() => setTheme((t) => (t === 'dark' ? 'light' : 'dark'))}
                    className={`relative w-10 h-5 rounded-full transition-colors duration-200 ${theme === 'dark' ? 'bg-indigo-500' : 'bg-slate-300'}`}
                  >
                    <span className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white shadow-sm transition-transform duration-200 ${theme === 'dark' ? 'translate-x-5' : 'translate-x-0'}`} />
                  </button>
                </div>
              </div>
              ...
              <div className="px-2 py-2">
                <button
                  ...
                  onClick={() => { setDropdownOpen(false); try { localStorage.clear(); } catch (e) {} window.location.reload(); }}
                >
                  <LogOut className="w-4 h-4 text-red-400 shrink-0" />
                  <span className="text-xs font-semibold text-red-600 dark:text-red-400">Log Out</span>
                </button>
              </div>
