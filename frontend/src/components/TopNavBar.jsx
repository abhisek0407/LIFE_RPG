...
//   const overallLevel = getEffectiveOverallLevel(user);
...
//           })}
//         </div> */}
//         {/* ── MIDDLE: Domain XP bars ── */}
//         <div className="hidden sm:flex flex-1 min-w-0 justify-center">
//           <div className="grid grid-cols-3 gap-1.5 md:gap-2 w-full max-w-xl">
//             {domainList.map(({ key, config, data }) => {
//               const pct = Math.min(
//                 100,
//                 Math.round(
...
//                   key={key}
//                   onClick={() => onDomainClick && onDomainClick(key)}
//                   className={`relative group cursor-pointer min-w-0 p-1.5 sm:p-2 md:p-2.5 rounded-xl border transition-all duration-200 bg-slate-50 dark:bg-rpg-card/90 shadow-sm hover:shadow-md ${
//                     key === "mental"
//                       ? "border-cyan-300 dark:border-cyan-500/30 hover:border-cyan-400 dark:hover:border-cyan-400/60"
//                       : key === "health"
//                         ? "border-emerald-300 dark:border-emerald-500/30 hover:border-emerald-400 dark:hover:border-emerald-400/60"
...
//                       <div
//                         className="p-0.5 sm:p-1 rounded shrink-0"
//                         style={{ color: config.color }}
//                       >
//                         <Icon className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
//                       </div>

//                       <span className="text-[9px] sm:text-[10px] md:text-xs font-semibold text-slate-700 dark:text-slate-200 truncate">
//                         {key === "skill" ? "Skill XP" : config.name}
//                       </span>
//                     </div>

//                     <span
//                       className="text-[8px] sm:text-[9px] md:text-[10px] font-bold px-1 sm:px-1.5 py-0.5 rounded font-mono shrink-0"
//                       style={{
//                         backgroundColor: `${config.color}22`,
//                         color: config.color,
//                       }}
...
//                   {/* Progress bar */}
//                   <div className="w-full bg-slate-200 dark:bg-slate-800/80 rounded-full h-1.5 sm:h-2 overflow-hidden">
//                     <div
//                       className="h-full rounded-full transition-all duration-500 ease-out"
//                       style={{
//                         width: `${pct}%`,
...
//                   {/* XP values */}
//                   <div className="flex justify-between mt-0.5 sm:mt-1 text-[8px] sm:text-[9px] md:text-[10px] text-slate-400 dark:text-slate-500 font-mono gap-1">
//                     <span className="truncate">{data.currentXp} XP</span>
//                     <span className="truncate">{data.xpToNextLevel} XP</span>
//                   </div>
...
//           >
//             <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-cyan-400 to-indigo-500 flex items-center justify-center border-2 border-cyan-400/50 dark:border-cyan-500/40 shrink-0">
...
//                 >
//                   <User className="w-4 h-4 text-cyan-500 dark:text-cyan-400 shrink-0" />
...
//                       Name, username, age & profile picture
//                     </p>
//                   </div>
//                 </button>
//               </div>
  ...
  return (
    <header className="sticky top-0 z-30 w-full bg-white/95 dark:bg-rpg-dark/95 backdrop-blur-md border-b border-slate-200 dark:border-rpg-border px-3 sm:px-4 lg:px-6 py-2 sm:py-2.5 shadow-sm dark:shadow-md transition-colors duration-200">
      <div className="flex items-center gap-1.5 sm:gap-2 lg:gap-4 min-w-0">
        ...
        {/* Compact pills: mobile AND tablet (< lg). Only the full stacked
            card is shown at lg+, so the middle domain section has room
            to breathe on tablets. */}
        ...
        {/* Full stacked card: desktop only (lg+) */}
        <div className="hidden lg:flex flex-col shrink-0 min-w-[96px] bg-slate-100 dark:bg-rpg-card/80 border border-slate-200 dark:border-rpg-border rounded-2xl px-3.5 py-2.5 gap-2.5">
          <div className="flex items-center gap-2">
            <Coins className="w-3.5 h-3.5 text-amber-500 dark:text-amber-400 fill-amber-500 dark:fill-amber-400 shrink-0" />
            <span className="font-mono font-bold text-xs text-amber-700 dark:text-amber-300 leading-none">
              {character?.gold ?? 0}
        ...
        {/* ── MIDDLE: Domain XP bars ──
            - sm to lg (tablet): compact — icon + level badge + bar only,
              no name text, no XP numbers. This is what was overflowing
              before; it's now deliberately minimal at this width.
            - lg+ (desktop): full card with name, XP numbers, tooltip. */}
        <div className="hidden sm:flex flex-1 min-w-0 justify-center">
          <div className="grid grid-cols-3 gap-1.5 md:gap-2 lg:gap-3 w-full max-w-xl">
            {domainList.map(({ key, config, data }) => {
              const pct = Math.min(
                100,
                Math.round(
              ...
              return (
                <div
                  key={key}
                  onClick={() => onDomainClick && onDomainClick(key)}
                  className={`relative group cursor-pointer min-w-0 p-1.5 lg:p-2.5 rounded-xl border transition-all duration-200 bg-slate-50 dark:bg-rpg-card/90 shadow-sm hover:shadow-md ${
                    key === "mental"
                      ? "border-cyan-300 dark:border-cyan-500/30 hover:border-cyan-400 dark:hover:border-cyan-400/60"
                      : key === "health"
                        ? "border-emerald-300 dark:border-emerald-500/30 hover:border-emerald-400 dark:hover:border-emerald-400/60"
                ...
                >
                  {/* Header */}
                  <div className="flex items-center justify-between gap-1 mb-1 lg:mb-1.5">
                    <div className="flex items-center gap-1 min-w-0">
                      <div
                        className="p-0.5 lg:p-1 rounded shrink-0"
                        style={{ color: config.color }}
                      >
                        <Icon className="w-3.5 h-3.5 lg:w-3.5 lg:h-3.5" />
                      </div>

                      {/* Name text: only shown once there's real room (lg+) */}
                      <span className="hidden lg:inline text-xs font-semibold text-slate-700 dark:text-slate-200 truncate">
                        {key === "skill" ? "Skill XP" : config.name}
                      </span>
                    </div>

                    <span
                      className="text-[9px] lg:text-[10px] font-bold px-1 lg:px-1.5 py-0.5 rounded font-mono shrink-0"
                      style={{
                        backgroundColor: `${config.color}22`,
                        color: config.color,
                      }}
                  ...
                  {/* Progress bar */}
                  <div className="w-full bg-slate-200 dark:bg-slate-800/80 rounded-full h-1.5 lg:h-2 overflow-hidden">
                    <div
                      className="h-full rounded-full transition-all duration-500 ease-out"
                      style={{
                        width: `${pct}%`,
                  ...
                  {/* XP values: only shown at lg+, where there's space */}
                  <div className="hidden lg:flex justify-between mt-1 text-[10px] text-slate-400 dark:text-slate-500 font-mono gap-1">
                    <span className="truncate">{data.currentXp} XP</span>
                    <span className="truncate">{data.xpToNextLevel} XP</span>
                  </div>

                  {/* Tooltip (also useful on tablet since name/XP are hidden there) */}
                  <div className="hidden group-hover:block absolute top-full left-0 mt-2 z-40 w-52 p-2.5 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-300 text-xs rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 pointer-events-none">
                    <p className="font-semibold text-slate-900 dark:text-white mb-0.5">
                      {key === "skill" ? "Skill XP" : config.name}
                    </p>
        ...
        <div className="relative shrink-0" ref={dropdownRef}>
          <button
            onClick={() => setDropdownOpen((v) => !v)}
            className="flex items-center gap-2 lg:gap-2.5 pl-1.5 lg:pl-2.5 pr-1.5 lg:pr-2 py-1 lg:py-1.5 rounded-2xl bg-slate-100 dark:bg-rpg-card/80 border border-slate-200 dark:border-rpg-border hover:border-cyan-400 dark:hover:border-cyan-500/60 transition-all"
          >
            <div className="w-8 h-8 lg:w-10 lg:h-10 rounded-full bg-gradient-to-br from-cyan-400 to-indigo-500 flex items-center justify-center border-2 border-cyan-400/50 dark:border-cyan-500/40 shrink-0">
              <UserCircle2 className="w-5 h-5 lg:w-6 lg:h-6 text-white" />
            </div>

            {/* Name — hidden on mobile AND tablet, only shown at lg+ */}
