...
export default function DailyQuestsView({ dailies, onToggleDaily, onAddDaily }) {
  ...
  // Photo proof is mandatory to complete a quest, so clicking the row or its
  // checkbox no longer completes it directly — it just opens the same
  // camera/photo picker as the "Proof" button. The quest is only ever
  // marked complete inside handleProofFileChange, once a photo is attached.
  const handleRowClick = (daily) => {
    if (daily.isCompletedToday) return;
    fileInputRefs.current[daily.id]?.click();
  };
  ...
  return (
    // pb-24 clears the fixed bottom tab bar (Home/Quests/Streak/.../Store)
    // on mobile so the last quest card — including a newly added one — is
    // never hidden underneath it. Not needed once the tab bar is gone (sm+).
    <div className="space-y-6 pb-24 sm:pb-0 transition-colors duration-200">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 sm:p-8 rounded-3xl bg-gradient-to-r from-cyan-50 via-slate-50 to-indigo-50 dark:from-cyan-950/40 dark:via-rpg-card dark:to-indigo-950/40 border border-cyan-200 dark:border-cyan-500/30 shadow-sm">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-mono font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-cyan-500/15 text-cyan-800 dark:text-cyan-300 border border-cyan-300 dark:border-cyan-500/30">
      ...
      <div className="space-y-3">
        {dailies.map((daily) => {
          ...
          return (
            ...
            >
              {/*
                Mobile-first: stack the "info" block and the "actions" block
                (flex-col) so the Proof/XP/Gold cluster wraps to its own row
                instead of being squeezed off-screen by shrink-0. From sm
                upward there's enough width to sit them side by side again.
              */}
              <div
                onClick={() => handleRowClick(daily)}
                className={`p-4 cursor-pointer flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 ${daily.isCompletedToday ? '' : 'hover:bg-slate-50 dark:hover:bg-rpg-cardHover'
                  }`}
              >
                <div className="flex items-center gap-3.5 min-w-0">
                  <button
                    ...
                    title={daily.isCompletedToday ? undefined : 'Photo proof required to complete'}
                  >
                    {daily.isCompletedToday ? (
                      <CheckCircle2 className="w-6 h-6 text-emerald-500 dark:text-emerald-400 shrink-0" />
                    ) : (
                ...
                <div
                  className="flex items-center gap-2 flex-wrap sm:flex-nowrap sm:shrink-0 pl-[52px] sm:pl-0"
                ...
                >
                  {!daily.isCompletedToday && (
                    <>
                      ...
                      <button
                        ...
                        onClick={(e) => handleAttachProofClick(e, daily.id)}
                        disabled={uploadingId === daily.id}
                        className="btn-tactile flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-cyan-300 dark:border-cyan-500/40 bg-cyan-50 dark:bg-cyan-900/30 hover:bg-cyan-100 dark:hover:bg-cyan-900/50 text-cyan-700 dark:text-cyan-300 text-[11px] font-bold transition-colors disabled:opacity-60 shrink-0"
                      >
                        {uploadingId === daily.id ? (
                          <>
                            <Loader2 className="w-3.5 h-3.5 animate-spin" />
                  ...
                  <span className="text-xs font-mono font-bold px-2.5 py-1 rounded-lg bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300 shrink-0">
                    +{daily.goldReward}g
                  </span>
                </div>
              </div>
